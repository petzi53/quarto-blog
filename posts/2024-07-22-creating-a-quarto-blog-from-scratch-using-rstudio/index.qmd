---
title: "Creating a Quarto blog from Scratch using RStudio"
subtitle: "Creating a Quarto blog from within RStudio. – A visual guide."
description:  |
...
I will not explain everything related to creating a Quarto blog;
instead, I will focus on my blog initialization choices.
...
window (Screenshot): Choose 'New Directory' to start an new project from scratch. ](creating-quarto-blog-1-min.png){#fig-new-project-wizard
...
![RStudio project type window (Screenshot): Choose 'Quarto Blog' to initialize a blog project
](creating-quarto-blog-2-min.png){#fig-new-blog-project-window
fig-alt="The Project Type dialog window lists the choices for new project. The first seven options are visible: New Project, R Package, Shiny Application, Quarto Project, Quarto Website, Quarto Blog, Quarto Book. (From top to the bottom. Top left is a button 'Back' for returning to the previous dialog window. Bottom right is a 'Cancel' button to stop the initialization process." fig-align="center"}

![RStudio Create Quarto Blog window (Screenshot): Fill the input fields with the appropriate data
](creating-quarto-blog-3-min.png){#fig-create-blog-window
...
-   Selecting "Use visual markdown editor" is not important, as this
    option is for all `.qmd` files in RStudio always available. I think that this option only controls
    the default behavior whenever a new file for a post is created.
...
![RStudio file listing window (Screenshot): Choose 'New Directory' to start an new project from scratch. ](generated-files-at-start-min.png){#fig-rstudio-file-listing-window
...
: Key files after initializing a Quarto blog {#tbl-quarto-blog-key-files}
...
![Source code of the blog home page 'index.qmd'-file within RStudio window (Screenshot) ](click-render-button-first-time-min.png){#fig-source-code-blog-home-page
...
![Website of the newly created Quarto blog (Screenshot): ](rendered-blog-first-time-min.png){#fig-blog-listing-start-page
